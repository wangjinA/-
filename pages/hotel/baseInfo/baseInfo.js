...
Page({
  ...
  setLocation({longitude, latitude, address}) {
    const wjForm = this.selectComponent('#wjForm')
    if (!wjForm.data.formData.address) {
      wjForm.data.formData.address = address
    }
    wjForm.data.formData.location = [longitude, latitude]
    wjForm.data.formList.forEach(item => {
      if(item.key === 'location'){
